...
public class AnimalController {
	...
	public ModelAndView editarAnimalMetodo(@PathVariable ("id") long idReq) {
		Animal animal = animalRepository.findById(idReq);
		List<Jaula> listarTodasJaulas = new ArrayList<>();
		Iterable<Jaula> todasJaulas = jaulaRepository.findAll();
		
		for (Jaula jaula : todasJaulas) {
			if (!jaula.equals(animal.getJaula())) {
				listarTodasJaulas.add(jaula);
			}
		}
		
		ModelAndView animalModelAndView = new ModelAndView("editarAnimal");
		animalModelAndView.addObject("animal", animal);
		animalModelAndView.addObject("jaulas", listarTodasJaulas);
