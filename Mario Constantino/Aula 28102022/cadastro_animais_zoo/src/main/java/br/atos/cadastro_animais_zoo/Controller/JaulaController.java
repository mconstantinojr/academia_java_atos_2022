...
public class JaulaController {
	...
	public ModelAndView editarJaulaMetodo(@PathVariable ("id") long idReq) {
		Jaula jaula = jaulaRepository.findById(idReq);
		List<Cuidador> listarTodosCuidadores = new ArrayList<>();
		List<Cuidador> cuidadoresDaJaula = jaula.getCuidadores();
		Iterable<Cuidador> todosCuidadores = cuidadorRepository.findAll();
		
		for (Cuidador cuidador : todosCuidadores) {
			if (cuidadoresDaJaula == null || (cuidadoresDaJaula != null && cuidadoresDaJaula.isEmpty()) ) {
				listarTodosCuidadores.add(cuidador);
			} else {
				for (Cuidador jaulaCuidador : cuidadoresDaJaula) {
					if (!cuidador.equals(jaulaCuidador)) {
						listarTodosCuidadores.add(cuidador);
					}
				}
			}
		...
		jaulaModelAndView.addObject("cuidadoresDaJaula", cuidadoresDaJaula);
		jaulaModelAndView.addObject("todosCuidadores", listarTodosCuidadores);
